...
evaluator -> evaluator: traverse continuation leaves\n(conservative evaluation because\nJobRunr does not expose the\ncontinuation trigger type)

evaluator -> evaluator: check if all considered\nleaf jobs are terminal\n(SUCCEEDED, FAILED, DELETED)
...
note right of evaluator
  Evaluation Rules:
  - The evaluator traverses all
    continuation leaves recursively
  - Because JobRunr does not expose
    continuation trigger metadata,
    visibility is conservative
  - Chain complete when all
    considered leaves are terminal
  - Chain failed if any considered
    leaf failed
end note
...
[NOTE]
====
JobRunr's public `AwaitingState` API does not expose whether a continuation was created via
`continueWith()` or `onFailure()`. The current implementation therefore evaluates chains
conservatively and may temporarily consider both continuation types as potentially relevant.
====
...
|All terminal leaf jobs considered by the evaluator succeeded
|Chain status = `SUCCEEDED`, isComplete = `true`

|Any terminal leaf job considered by the evaluator failed
|Chain status = `FAILED`, isComplete = `true`

|Any considered leaf job still running (ENQUEUED, PROCESSING, PROCESSED)
|Chain status = `PROCESSING`, isComplete = `false`
|===

.Continuation Handling
The evaluator traverses all continuation leaves recursively. Because the public JobRunr API does
not expose the continuation trigger type, the current implementation cannot perfectly distinguish
between `continueWith()` and `onFailure()` leaves and therefore prefers conservative visibility
over under-reporting incomplete chains.
