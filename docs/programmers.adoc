...
public class PrintDashboardBranding implements JobRunrControlUiBrandingProvider {
    ...
    public JobRunrControlUiBranding branding() {
        return new JobRunrControlUiBranding(
...
the JobRunr Control UI roles. A host provider with an empty `requiredRoles` set displays the
host application's branding to all authenticated users. The default stage can be configured
with `quarkus.jobrunr-control.ui.stage=DEV|INT|VPR|PRD`; an empty or missing value hides the badge.
