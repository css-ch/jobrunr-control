...
participant "CalculationBatchJobPreparer" as Preparer
...
BatchJob -> Scheduler: BackgroundJobRequest.startBatch(preparerRequest)
note right
    Creates batch parent if not exists
end note
Scheduler -> DB: INSERT/UPDATE batch parent
Scheduler -> Preparer: run(preparerRequest)

Preparer -> Preparer: Load items to process\n(e.g., 1000 items)
Preparer -> Preparer: Sleep 5s (simulate preparation)
Preparer -> Scheduler: BackgroundJobRequest.enqueue(items.stream())
note right
...
Preparer --> Scheduler: Completed
Scheduler -> DB: UPDATE preparer job (SUCCEEDED)
...
2. **Batch Job Executes**: `CalculationBatchJob.run(request)` is called
3. **Batch Parent Created**: Inside the job, `BackgroundJobRequest.startBatch(preparerRequest)` is called
4. **Preparer Job Enqueued**: The preparer job is enqueued with the batch parent context
5. **Child Jobs Created**: Preparer creates all child jobs linked to the batch parent
...
public class CalculationBatchJob implements ConfigurableJob<CalculationBatchJobRequest> {
    ...
    public void run(CalculationBatchJobRequest request) throws Exception {
        ...
        // This creates the batch parent and enqueues the preparer
...
.Disadvantages
* ❌ Scheduled batch jobs have two-level indirection (wrapper → actual batch)
