...
CREATE TABLE IF NOT EXISTS `JOBRUNR_CONTROL_BATCH_MESSAGES` (
    ...
    `ATTEMPT_NR` INT NOT NULL DEFAULT 0,
...
CREATE INDEX idx_batch_msg_latest ON `JOBRUNR_CONTROL_BATCH_MESSAGES`(`BATCH_JOB_ID`, `IS_LATEST`, `CREATED_AT`, `ID`);
CREATE INDEX idx_batch_msg_attempt ON `JOBRUNR_CONTROL_BATCH_MESSAGES`(`BATCH_JOB_ID`, `CHILD_JOB_ID`, `IS_LATEST`, `ATTEMPT_NR`);
